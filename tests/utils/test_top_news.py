...
with patch('database.init_db.init_db'), \
    ...
    from utils.top_news import (cache_top_news_items, get_cached_top_news_item,
                                get_top_news)
...
def test_get_top_news():
    ...
    )][:3] == get_top_news(sentences, important_news, 3)

    assert [item['news'] for item in important_news.values(
    )][:1] == get_top_news(sentences, important_news, 1)
