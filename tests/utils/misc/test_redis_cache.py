...
def test_exists():
    with patch(
        'utils.misc.redis_cache.redis_connection'
    ) as redis_connection_mock:
        key = '1:2:3:text'
        cache.exists(key)
...
@pytest.mark.parametrize(
    'prefix, search_query, date_from, date_to, result',
    [
        (
            'prefix1',
            'Ecology',
...
def test_key_query(
    prefix: str, search_query: str, date_from: date, date_to: date, result: str
):
...
@pytest.mark.parametrize(
    'prefix, search_query, date_from, date_to, text, ttl, expected_key',
    [
        (
            'summary_input',
            'Russia',
        ...
        (
            ...
            'text2',
            3000,
            'summary:Ecology:2023-04-09T00:00:00:2023-04-09T23:59:59',
        ),
        ('', '', '', '', 'text3', 200, ':::'),
    ],
)
...
def test_get_set():
    ...
    with patch('utils.misc.redis_cache.get') as get_mock, patch(
        'utils.misc.redis_cache.set'
    ) as set_mock, patch(
        'utils.misc.redis_cache.redis_connection.exists'
    ) as exists_mock:
    ...
    with patch('utils.misc.redis_cache.get') as get_mock, patch(
        ...
        'utils.misc.redis_cache.redis_connection.exists', return_value=False
    ) as exists_mock:
        key, ttl = 'a:b:c:d', 1000
        cache.get_set(key, ttl, func, 'text')
        exists_mock.assert_called_once_with(key)
