...
def get_results(
    ...
    text_msg = (
        f'*Getting news from {date_from_str} to {date_to_str} for '
    ...
    try:
        ...
        if news_count == 0:
            ...
            bot.send_message(
                chat_id, '*No news found.*', parse_mode='Markdown'
            )
            return

        text_msg = (
            f'*Total news found: {news_count}. '
            f'Generating summary and top news...*'
        )
        bot.send_message(chat_id, text_msg, parse_mode='Markdown')

        summary, top_news = _get_summary_and_top_news(
            search_query, date_from, date_to, summary_input, important_news
        )
        ...
        if summary and top_news:
            ...
            bot.send_message(
                chat_id, '*Unable to get top news.*', parse_mode='Markdown'
            )
    except (
        ...
        if isinstance(exception, ValueError):
            ...
            bot.send_message(
                chat_id, '*Error. Invalid data.*', parse_mode='Markdown'
            )
        else:
            logger.error(
            ...
            bot.send_message(
                chat_id, '*Some error occurred.*', parse_mode='Markdown'
            )


def _get_summary_and_top_news(
    search_query: str,
    date_from: date,
    date_to: date,
    summary_input: str,
...
def _display_summary_and_top_news(
    chat_id: str, summary: list[str], top_news: list[dict]
) -> None:
    """
    ...
    text_msg = (
        '*Here are top news. You can choose one to get '
        'its summary or read the full article.*'
    )
    bot.send_message(
        chat_id,
        text_msg,
        reply_markup=news_menu.main(top_news),
        parse_mode='Markdown',
    )
