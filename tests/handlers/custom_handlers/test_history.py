...
def test_bot_history(
    ...
    assert (
        mock_bot_history.send_message.call_args[1]['parse_mode'] == 'Markdown'
    )
    assert (
        mock_bot_history.send_message.call_args[1]['reply_markup'] is not None
    )
...
def test_bot_history_item(
    ...
    mock_get_results.assert_called_once_with(
        ...
        history_item.query,
        history_item.date_from,
        history_item.date_to,
    )
...
@patch('handlers.custom_handlers.history.SearchHistory')
@patch('handlers.custom_handlers.history.get_results')
def test_bot_history_item_not_found(
    mock_get_results,
    mock_history,
    mock_bot_news_results,
    mock_bot_history,
