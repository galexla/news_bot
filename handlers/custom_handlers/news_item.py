...
@bot.callback_query_handler(
    func=lambda call: call.data.startswith('news_'), state=None
)
...
@bot.callback_query_handler(
    func=lambda call: call.data.startswith('summary_'), state=None
)
def bot_news_summary(call: CallbackQuery):
    ...
    if news_item:
        ...
        summary = cache.get_set(
            cache.key('article_summary', news_id),
            ttl,
            get_summary,
            news_item[config.NEWS_BODY],
        )
        ...
        bot.send_message(
            chat_id, '*Unable to get news summary.*', parse_mode='Markdown'
        )
