...
with patch('database.init_db.init_db'), patch('database.init_db.create_tables'), \
        patch('loader.TeleBot'), patch('loader.redis.Redis'), patch('loader.loguru.logger.add'), \
...
def test_initialization(mock_create_tables, mock_init_db, mock_redis,
                        mock_storage, mock_bot, mock_logger_add, mock_setLevel):
    ...
    mock_redis.assert_called_once_with(host=loader.config.REDIS_HOST, port=loader.config.REDIS_PORT,
                                       db=loader.config.REDIS_DB, password=loader.config.REDIS_PASSWORD,
                                       decode_responses=True, encoding='utf-8',
                                       socket_timeout=None, health_check_interval=10)

    mock_init_db.assert_called_once_with(loader.config.POSTGRES_DB, host=loader.config.POSTGRES_HOST,
                                         user=loader.config.POSTGRES_USER, password=loader.config.POSTGRES_PASSWORD,
