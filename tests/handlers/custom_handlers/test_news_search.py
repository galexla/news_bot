...
@patch('handlers.custom_handlers.news_search.bot')
def test__handle_invalid_input(
    bot_mock, mock_manager, message, invalid_inputs
):
    ...
    bot_mock.retrieve_data.assert_called_once_with(message.from_user.id)
    bot_mock.delete_state.assert_called_once_with(message.from_user.id)
    bot_mock.reply_to.assert_called_once()


@pytest.mark.parametrize('invalid_inputs', [0, 1])
@patch('handlers.custom_handlers.news_search.bot')
def test__handle_invalid_input_valid(
    bot_mock, mock_manager, message, invalid_inputs
):
    bot_mock.retrieve_data.return_value = mock_manager(invalid_inputs)
...
def test__display_calendar(calendar_mock, bot_mock, lstep_mock):
    ...
    assert (
        bot_mock.send_message.call_args[1]['reply_markup']
        == calendar_mock().build.return_value[0]
    )
