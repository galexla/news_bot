...
@pytest.mark.parametrize('all_cache_exist, cache_exists, news_count, summary_input, important_news', [
    (True, True, 1, 'summary input', {
     'a': {'importance': 1}, 'b': {'importance': 2}}),
    (True, True, 5, 'summary input 2', {
     'a': {'importance': 3}, 'b': {'importance': 4}}),
    (False, True, 5, 'summary input 2', {
     'a': {'importance': 3}, 'b': {'importance': 4}}),
    (False, False, 5, 'summary input 2', {
     'a': {'importance': 3}, 'b': {'importance': 4}}),
    (False, False, 0, '', {}),
    (True, True, 0, '', {})
])
...
def test_get_news_semimanufactures(mock_get_summary_input, mock_get_important_news,
                                   mock_news_api, mock_cache, all_cache_exist,
                                   cache_exists, news_count, summary_input,
                                   important_news):
    mock_cache.all_exist.return_value = all_cache_exist
    cache_get_values = {'news_count': news_count, 'summary_input': summary_input,
                        'important_news': important_news}
    mock_cache.get.side_effect = lambda key: cache_get_values[key.split(':')[
        0]]
    ...
    actual_news_count, actual_summary_input, actual_important_news = \
