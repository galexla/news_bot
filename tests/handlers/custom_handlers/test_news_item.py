...
with patch('database.init_db.init_db'), \
    ...
    from handlers.custom_handlers.news_item import (bot_click_news_item,
                                                    bot_news_summary)
...
def callback_query():
    user = User(1, False, first_name='test_user')
    message = Message(1, user, 0, Chat(1, 'private'), '', {}, '{}')
    query = CallbackQuery('1', user, data='news_123', chat_instance='private',
...
def callback_query2():
    user = User(1, False, first_name='test_user')
    message = Message(1, user, 0, Chat(1, 'private'), '', {}, '{}')
    query = CallbackQuery('1', user, data='summary_1', chat_instance='private',
                          json_string='{}', message=message)
    return query
...
def test_bot_news_summary(mock_get_summary, mock_cache, mock_top_news,
    ...
    mock_bot.send_message.assert_called_once()
    assert mock_bot.send_message.call_args[0][0] == callback_query2.message.chat.id
...
def test_bot_news_summary_error(mock_get_summary, mock_cache, mock_top_news,
    ...
    mock_bot.send_message.assert_called_once()
    assert mock_bot.send_message.call_args[0][0] == callback_query2.message.chat.id
    assert mock_bot.send_message.call_args[0][1] == '*Unable to get news summary.*'
    assert mock_bot.send_message.call_args[1]['parse_mode'] == 'Markdown'
